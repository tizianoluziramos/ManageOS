...
export default class UserManager {
  ...
  private static execCommand(command: string): CommandResult {
    try {
  ...
  static getLastLogon(username: string): CommandResult {
    ...
    const res = this.execCommand(
      `powershell -Command "Get-LocalUser -Name '${escUser}' | Select-Object -ExpandProperty LastLogon"`
    );

    if (!res.success || !res.output) return res;
  ...
  static setUserActive(username: string, active: boolean): CommandResult {
    const escUser = this.escapeArg(username);
    const res = this.execCommand(
      `net user ${escUser} /active:${active ? "yes" : "no"}`
    );
    if (res.success) delete this.cache.users;
    return res;
  }

  static requirePasswordChange(
    username: string,
    require: boolean
  ): CommandResult {
    const escUser = this.escapeArg(username);
    return this.execCommand(
      `net user ${escUser} /logonpasswordchg:${require ? "yes" : "no"}`
    );
  }
  ...
  static removeUserFromGroup(username: string, group: string): CommandResult {
    ...
    const escGroup = this.escapeArg(group);
    const res = this.execCommand(
      `net localgroup ${escGroup} ${escUser} /delete`
    );
    if (res.success) delete this.cache.groups;
    return res;
  }
  ...
  static getUserInfo(username: string): UserInfo | null {
    ...
    for (let raw of lines) {
      ...
      if (line.startsWith("User name")) collecting = true;
      if (!collecting) continue;
  ...
  static getAllGroupMembers(): { group: string; members: string[] }[] {
    const groups = this.listGroups();
    const result: { group: string; members: string[] }[] = [];
  ...
  static listActiveSessions(): string[] {
    const res = this.execCommand(
      'powershell -Command "Get-LocalUser | Where-Object {$_.Enabled -eq $true} | Select-Object -ExpandProperty Name"'
    );
