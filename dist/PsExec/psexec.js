...
const PS_EXEC_PATH = path.join(__dirname, "dependencies", ARCH === "x64" ? "PsExec64.exe" : "PsExec.exe");
class Sync {
    ...
    static buildArgs(cmd, options) {
        let args = "";
        if (options?.computer)
            args += `\\\\${options.computer} `;
        if (options?.fileList)
            args += `@${options.fileList} `;
        if (options?.user)
            args += `-u ${options.user} `;
        if (options?.password)
            args += `-p ${options.password} `;
        if (options?.timeout)
            args += `-n ${options.timeout} `;
        if (options?.limited)
            args += "-l ";
        if (options?.system)
            args += "-s ";
        ...
        if (options?.workingDir)
            args += `-w "${options.workingDir}" `;
        if (options?.winlogon)
            args += "-x ";
        if (options?.priority)
            args += `-${options.priority} `;
        if (options?.processors)
            args += `-a ${options.processors.join(",")} `;
        args += cmd;
        return args;
    }
}
...
//# sourceMappingURL=psexec.js.map